...
$tablestudentrow = mysqli_fetch_assoc($tablestudentsrun);

$studNum = $tablestudentrow['ReferenceNo'];

$getFeedback = "SELECT * FROM feedbacks INNER JOIN Student_Accounts ON feedbacks.ReferenceNo = Student_Accounts.ReferenceNo WHERE Student_Accounts.Course_ID = '$courses'";
$getFeedbackrun = mysqli_query($conn, $getFeedback);
?>
        ...
        <div class="col-sm-2">
            ...
            <div class="col-sm-10">
                <div class="page-title">
                    <h1>FEEDBACKS</h1>
                    <a href="adminfeedback.php"><button class="go-back-btn">GO BACK</button></a>
                </div>
                    ...
                    <table>
                        ...
                        <?php
                            if(isset($_POST['search'])){
                                $tablestudentsrun = mysqli_query($conn, $tablestudents);
                                $search = $_POST['search'];
                                $finder = "SELECT * FROM feedbacks INNER JOIN Student_Accounts ON feedbacks.ReferenceNo = Student_Accounts.ReferenceNo WHERE Student_Accounts.Course_ID = '$courses' AND FirstName LIKE '$search%' OR LastName LIKE '$search%'";
                                ...
                                if($search != "" && mysqli_num_rows($finderrun) > 0){
                                    while($finderrows = mysqli_fetch_assoc($finderrun)){
                                        ...
                                        echo"
                                                ...
                                                <td>{$finderrows['FeedbackDate']}</td>
                                                <td>{$finderrows['fb_subject']}</td>
                                                <td>{$finderrows['fb_content']}</td>
                                                <td>{$finderrows['ReferenceNo']}</td>
                                                <td class='table-btn'><form method='POST' action='view.php'><input type='text' name='feedbackGo' hidden value='$ref'><button type='submit' class='views'>VIEW</button></form></td>
                                                </tr>";
                                    }
                                }
                                if($search == ""){
                                    while($getFeedbackrow = mysqli_fetch_assoc($getFeedbackrun)){
                                        $ref = $getFeedbackrow['ReferenceNo'];
                                        echo"
                                                <tr>
                                                <td>{$getFeedbackrow['FirstName']} {$getFeedbackrow['LastName']}</td>
                                                <td>{$getFeedbackrow['FeedbackDate']}</td>
                                                <td>{$getFeedbackrow['fb_subject']}</td>
                                                <td>{$getFeedbackrow['fb_content']}</td>
                                                <td>{$getFeedbackrow['ReferenceNo']}</td>
                                                <td class='table-btn'><form method='POST' action='view.php'><input type='text' name='feedbackGo' hidden value='$ref'><button type='submit' class='views'>VIEW</button></form></td>
                                                </tr>";
                                        }
                                }
